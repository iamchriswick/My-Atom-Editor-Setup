...
import { AppendOpcode, Slice, UpdatingOpcode, Constants, ConstantString, ConstantSlice } from '../opcodes';
...
export default class VM implements PublicVM {
    ...
    frame: FrameStack;
    constants: Constants;
    static initial(env: Environment, self: PathReference<Opaque>, dynamicScope: DynamicScope, elementStack: ElementStack, size: number): VM;
    ...
    enter(sliceId: ConstantSlice): void;
    enterWithKey(key: string, ops: Slice): void;
    enterList(ops: Slice): void;
    ...
    pushEvalFrame(ops: AppendOpcode[]): void;
    pushChildScope(): void;
    ...
    evaluateOpcode(opcode: AppendOpcode): void;
